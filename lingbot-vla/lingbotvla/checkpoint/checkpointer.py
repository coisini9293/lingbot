...
def prune_old_checkpoints(checkpoint_root: str, keep: int) -> None:
    """Delete oldest global_step_* dirs, keeping at most ``keep`` newest."""
    if keep <= 0:
        return
    import shutil

    ckpt_dirs = sorted(
        (p for p in Path(checkpoint_root).glob("global_step_*") if p.is_dir()),
        key=lambda p: int(p.name.rsplit("_", 1)[-1]),
    )
    for old in ckpt_dirs[:-keep]:
        shutil.rmtree(old, ignore_errors=True)
        logger.info_rank0(f"Pruned old checkpoint: {old}")
