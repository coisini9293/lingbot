...
class DataArguments:
    ...
    video_backend: Literal["pyav", "torchcodec"] = field(
        default="torchcodec",
        metadata={
            "help": "Video decode backend. torchcodec=faster CPU decode; pyav=fallback."
        },
    )
...
class TrainingArguments:
    ...
    max_checkpoints_to_keep: int = field(
        default=1,
        metadata={
            "help": "Keep at most N global_step_* checkpoint directories; prune older ones after each save."
        },
    )
    save_optimizer: bool = field(
        default=False,
        metadata={
            "help": "Save optimizer state in DCP checkpoint (~25GB). Disable for deploy-only finetune to save disk."
        },
    )
    seed: int = field(
        default=42,
