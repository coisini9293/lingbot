...
from lingbotvla.checkpoint import build_checkpointer, ckpt_to_state_dict, prune_old_checkpoints
...
def _maybe_prune_checkpoints(args) -> None:
    keep = args.train.max_checkpoints_to_keep
    if args.train.global_rank == 0 and keep > 0:
        prune_old_checkpoints(args.train.save_checkpoint_path, keep)
...
def main():
    ...
    if is_windows:
        ...
        if not dist.is_initialized():
            dist.init_process_group(backend="gloo", rank=0, world_size=1, init_method="tcp://127.0.0.1:29500")
    else:
